...
def get_type_data(ptr, index: int, data_type: str) -> int:
...
def set_type_data(ptr, index: int, data, data_type: str) -> None:
    if data_type == "uint64_t":
...
def __ROR__(value: ctypes.c_ulonglong, count: int) -> ctypes.c_ulonglong:
    ...
    value = ctypes.c_ulonglong(value.value >> count).value # pyright: ignore[reportAssignmentType]
    value = value | low # pyright: ignore[reportOperatorIssue]
    return value


def encrypt_ladon_input(hash_table: bytearray, input_data: bytearray) -> bytes:
...
def encrypt_ladon(md5hex: bytes, data: bytes, size: int) -> bytearray:
    ...
    for i in range(0, 0x22):
        x9 = buffer_b0
        x8 = buffer_b8
        x8 = validate(__ROR__(ctypes.c_ulonglong(x8), 8)) # pyright: ignore[reportArgumentType]
        x8 = validate(x8 + x9)
        x8 = validate(x8 ^ i)
        temp.append(x8)
    ...
    for i in range(new_size // 16):
        output[i * 16 : (i + 1) * 16] = encrypt_ladon_input(hash_table, input[i * 16 : (i + 1) * 16])
...
class XLadon:
    @staticmethod
    def calculate(timestamp: int, license_id: int = 1611921764, aid: int = 1340) -> str:

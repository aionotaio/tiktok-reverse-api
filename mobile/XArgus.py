...
class XArgus:
    @staticmethod
    def encrypt_enc_pb(data: Any, l: int) -> bytes:
        data = list(data)
        xor_array = data[:8]
    ...
    @staticmethod
    def calculate_constant(code: str) -> int:
    ...
    @staticmethod
    def encrypt(argus_bean: dict[int, Any]) -> str:
        protobuf = pad(bytes.fromhex(ProtoBuf(argus_bean).toBuf().hex()), block_size)
        ...
        b_buffer = XArgus.encrypt_enc_pb((b"\xf2\xf7\xfc\xff\xf2\xf7\xfc\xff" + enc_pb), new_len + 8)
    ...
    @staticmethod
    def calculate(query_string: str | None = None, stub: None | str = None, timestamp: int = int(time()), aid: int = 1340, license_id: int = 1611921764, platform: int = 0, channel="googleplay", sec_device_id: str | None = None, sdk_version: str = "v04.04.05-ov-android", sdk_version_int: int = 134744640, lanusk=None, lanusk_version=None, seed_token=None, seed_version=None) -> str:
        ...
        argus_bean = {
            ...
            13: XArgus.get_bodyhash(stub),  # bodyHash
            14: XArgus.get_queryhash(str(query_string)),  # queryHash
            # 15: {
        ...
        if lanusk and lanusk_version:
            ...
            argus_bean[19] = SM3().sm3_hash(
                bytes.fromhex(
                    str(query_string).encode("utf-8").hex()
                    + str(stub)
        ...
        argus_bean[23] = {
            ...
            3: channel,
            4: XArgus.calculate_constant(params_dict.get("os_version", [""])[0]),
        }
        ...
        return XArgus.encrypt(argus_bean)
